<template>
  <div class="board">
    <div v-if="board">
      ...
      <div class="row">
        <!--Container List-->
        <div v-for="container in containers" v-bind:key="container.id" class="col"> 
          <Container :containerProp="container"/>
        </div>
        <!--New Container-->
        <div class="col">
          <div class="container-create">
            <input type="text" v-model="newContainer.name" placeholder="New Container"/>
            <button class="btn btn-success" v-on:click="createContainer">Create</button>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    createContainer()
    {
      ...
      this.newContainer["board"] = { url: this.board.url, id: this.board.id, name: this.board.name };
      this.newContainer["archived"] = false;
      this.newContainer["position"] = 7;
      this.$store.dispatch("createContainerAsync", this.newContainer);
      // reset new container
      this.newContainer = { name: "", board: { url: this.board.url, id: this.board.id, name: this.board.name  }, archived: false, position: 0 }
    }
  },
  components: {
    Container
